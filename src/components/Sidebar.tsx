  ...
  return (
    ...
    >
      <button
        ...
        className={`w-full text-left px-2 py-1.5 md:px-3 md:py-2.5 rounded-lg transition-all flex items-center justify-between cursor-grab active:cursor-grabbing ${
      ...
      >
        <div className="flex-1 min-w-0">
          <p className="text-xs md:text-sm font-bold truncate text-gray-950">{task.title}</p>
...
export function Sidebar({ selectedTaskId, onTaskSelect }: SidebarProps) {
  ...
  if (!selectedCategoryId) {
    return (
      <div className="w-full md:w-64 bg-gray-50 border-r-2 border-gray-400 md:border-r-2 md:border-gray-400 p-3 md:p-6 flex items-center justify-center">
        <p className="text-gray-500 text-center text-sm md:text-base">카테고리를 선택해주세요</p>
      </div>
    );
  }
  ...
  return (
    <div className="w-full md:w-64 bg-white border-r-2 border-gray-400 md:border-r-2 md:border-gray-400 flex flex-col overflow-hidden">
      {/* 헤더 */}
      <div className="px-3 py-2 md:p-4 border-b-2 border-gray-300 bg-orange-100">
        <h2 className="font-bold text-gray-900 text-xs md:text-sm">작업 항목 ({tasks.length})</h2>
      </div>
      ...
      >
        <div className="flex-1 overflow-y-auto">
          <SortableContext items={tasks.map(t => t.id)} strategy={verticalListSortingStrategy}>
            <div className="space-y-1 md:space-y-2 px-2 py-2 md:p-3">
              {tasks.length === 0 ? (
      ...
      {/* 추가 입력란 */}
      <div className="px-2 py-2 md:p-4 border-t-2 border-gray-300 bg-gray-50">
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 px-2 py-1 md:px-3 md:py-2 border-2 border-gray-400 rounded-lg text-xs md:text-sm bg-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all"
          />
          <button
            onClick={handleAddTask}
            disabled={!newTaskName.trim()}
            className="px-2 py-1 md:px-3 md:py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 disabled:bg-gray-300 disabled:cursor-not-allowed transition-colors font-medium flex items-center justify-center flex-shrink-0"
            title="항목 추가"
          >
